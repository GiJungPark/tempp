...
Swagger에는 기획서 기준 사용자 입력값만 노출한다. 전산매체 규격상 필요하지만 사용자가 직접 입력하지 않는 값은 아래 순서로 채운다.
...
| 신고구분상세코드 | 기본 `01` 정기신고 |
| 지급연도/지급월 | 첫 번째 소득자 `paymentDate`, 없으면 `attributionYm`에서 파생 |
| 반기 코드 | 지급월 1~6월이면 `1`, 7~12월이면 `2` |
| 세율/필요경비/지급건수 | 전산매체 규격과 기획 기본 정책으로 계산 |

내부 구현에서는 테스트/디버깅용으로 `businessPlace`를 body에 직접 전달하면 그 값을 우선 사용한다. 공개 Swagger에는 숨겨져 있으며, 값이 없으면 홈택스 사업장 조회 원문에서 사업자번호/상호/대표자/주소/전화/세무서코드 후보를 추출한다. `taxOfficeCode`가 없으면 `businessPlace.address`로 관할 세무서 코드를 자동 추정한다.
