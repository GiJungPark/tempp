...
export class IncomePaymentRecipientDto {
  ...
  @ApiPropertyOptional({ description: '내외국인 구분. 1=내국인, 9=외국인', enum: ['1', '9'], example: '1' })
  foreignerYn?: ForeignerYn;

  @ApiProperty({ description: '귀속연월 YYYYMM', example: '202605' })
  attributionYm!: string;

  @ApiPropertyOptional({ description: '지급일자 YYYYMMDD. 연간 지급명세서 지급연도 산출에 사용', example: '20260531' })
  paymentDate?: string;

  @ApiPropertyOptional({ description: '입력 금액 기준. gross=세전, net=세후', enum: ['gross', 'net'], example: 'gross' })
  amountInputType?: AmountInputType;

  @ApiProperty({ description: '입력 금액. amountInputType 기준으로 해석', example: 1000000 })
  amount!: number;

  @ApiPropertyOptional({ description: '지급건수. 없으면 1', example: 1 })
  paymentCount?: number;
}
...
export class BusinessIncomePaymentRecipientDto extends IncomePaymentRecipientDto {
  ...
  @ApiPropertyOptional({ description: '사업소득 세율. 없으면 업종코드 기준 기본값 사용', enum: [3, 5, 20], example: 3 })
  taxRate?: 3 | 5 | 20;

  @ApiPropertyOptional({ description: '소득자 사업자번호. 병의원 업종 851101 등에서 사용' })
  recipientBusinessNo?: string;

  @ApiPropertyOptional({ description: '소득자 상호. 연간 사업소득 지급명세서에서 사용' })
...
export class OtherIncomePaymentRecipientDto extends IncomePaymentRecipientDto {
  ...
  @ApiPropertyOptional({ description: '필요경비율. 없으면 0.6', example: 0.6 })
  necessaryExpenseRate?: number;

  @ApiPropertyOptional({ description: '필요경비 금액을 직접 지정할 때 사용' })
  necessaryExpense?: number;

  @ApiPropertyOptional({ description: '기타소득 세율. 없으면 소득구분코드 기준 기본값 사용', enum: [20, 15, 0, 30], example: 20 })
  taxRate?: 20 | 15 | 0 | 30;
}

// 원천징수이행상황신고서 C103900 생성 요청.
export class GenerateWithholdingTaxFileDto {
  @ApiProperty({ description: '사업장/제출자 정보', type: ElectronicBusinessPlaceDto })
  ...
  @ApiPropertyOptional({ description: '제출연월 YYYYMM. 없으면 paymentYm + 1', example: '202606' })
  submitYm?: string;

  @ApiPropertyOptional({ description: '작성일자 YYYYMMDD. 없으면 서버 오늘 날짜', example: '20260629' })
  writtenDate?: string;

  @ApiPropertyOptional({ description: '신고구분상세코드. 01=정기, 02=수정, 03=기한후', enum: ['01', '02', '03'], example: '01' })
...
export class GenerateSimpleBusinessIncomeStatementFileDto {
  ...
  @ApiProperty({ description: '지급연도 YYYY', example: '2026' })
  paymentYear!: string;

  @ApiProperty({ description: '지급월 MM', example: '05' })
  paymentMonth!: string;

  @ApiPropertyOptional({ description: '지급시기. 1=상반기, 2=하반기. 없으면 지급월 기준 자동', enum: ['1', '2'], example: '1' })
  paymentHalf?: '1' | '2';

  @ApiPropertyOptional({ description: '제출연월일 YYYYMMDD. 없으면 서버 오늘 날짜', example: '20260629' })
  submitDate?: string;
...
export class GenerateAnnualBusinessIncomeStatementFileDto {
  ...
  @ApiProperty({ description: '귀속연도 YYYY', example: '2026' })
  attributionYear!: string;

  @ApiPropertyOptional({ description: '제출연월일 YYYYMMDD. 없으면 서버 오늘 날짜', example: '20270228' })
  submitDate?: string;

  @ApiPropertyOptional({ description: '파일명 prefix. 사업소득 지급명세서는 샘플 기준 F', example: 'F' })
  fileNamePrefix?: string;

  @ApiPropertyOptional({ description: '제출대상기간코드. 1=연간 합산, 2=휴폐업 수시, 3=수시 분할', enum: ['1', '2', '3'], example: '1' })
...
export class GenerateAnnualOtherIncomeStatementFileDto {
  ...
  @ApiPropertyOptional({ description: '파일명 prefix. 기타소득 지급명세서는 원문 확인 전까지 override 용도', example: 'G' })
  fileNamePrefix?: string;
